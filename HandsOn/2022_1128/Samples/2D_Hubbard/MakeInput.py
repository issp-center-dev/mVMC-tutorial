...
def main():
    ...
    OutputStdFace(list_org,list_sub)
    #[s] output StdFace files
    #[s] output StdFace files
    OutputGreen(list_org)
    #[s] output StdFace files

def OutputStdFace(list_org,list_sub):
    Lx      = list_org[0]
    Ly      = list_org[1]
    Lz      = list_org[2]
    orb_num = list_org[3]
    ...
    with open("StdFace.def", 'w') as f:
        print("W             = %d       "%(Lx),file=f)
        print("L             = %d       "%(Ly),file=f)
        print("Wsub          = %d       "%(sub_x),file=f)
        print("Lsub          = %d       "%(sub_y),file=f)
        print("model         = \"Hubbard\" ",file=f)
        print("lattice       = \"square\"",file=f)
        print("t             = 1.0      ",file=f)
        print("U             = 4.0      ",file=f)
