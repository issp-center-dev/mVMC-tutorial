...
#
      ...
      $S_A    = $S_A/(1.0*$Sample);
      $S_E    = sqrt(abs($S_D/(1.0*($Sample-1.0))-$Sample/($Sample-1.0)*$S_A**2))/(sqrt(1.0*$Sample));
      $Sxy_A  = $Sxy_A/(1.0*$Sample);
      $Sxy_E  = sqrt(abs($Sxy_D/(1.0*($Sample-1.0))-$Sample/($Sample-1.0)*$Sxy_A**2))/(sqrt(1.0*$Sample));
      $Sz_A   = $Sz_A/(1.0*$Sample);
      $Sz_E   = sqrt(abs($Sz_D/(1.0*($Sample-1.0))-$Sample/($Sample-1.0)*$Sz_A**2))/(sqrt(1.0*$Sample));
